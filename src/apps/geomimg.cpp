...
int main(int argc, const char* const argv[])
{
    ...
    // Compute geometry image
    StartTimer();
    dfy::GImage Img(M, CLIArgs.Resolution);
    Img.Compute(UV, CM.Triangles());
    if (CLIArgs.Verbosity > 1)
    {
        std::cout << "Computed geometry image with resolution ";
        std::cout << Img.GetWidth() << "-by-" << Img.GetHeight();
        std::cout << " in " << StopTimer() << " seconds.";
        std::cout << std::endl;
    }
    ...
    StartTimer();
    dfy::ExportGImage(CLIArgs.OutputFile, Img);
    if (CLIArgs.Verbosity > 1)
    {
        std::cout << "Geometry image exported to " << CLIArgs.OutputFile << " in ";
        std::cout << StopTimer() << " seconds.";
        std::cout << std::endl;
    }
...
void ParseArgs(int argc, const char *const argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        // Geometry image resolution
        if (Argvi == "-r" || Argvi == "--resolution")
        {
    ...
    // If no output mesh is given, use input mesh with "-gimage" as suffix
    if (CLIArgs.OutputFile.empty())
    {
        std::filesystem::path InPath(CLIArgs.InputFile);
        InPath.replace_filename(InPath.stem().string() + "-gimage.png");
    ...
    if (Ext != ".png")
    {
        std::cerr << "Output filename has no PNG extension (";
...
void Usage(const std::string &argv0)
{
    ...
    std::cout << "  " << argv0 << " input_mesh [-n num_samples] [-d disk_samples] [-m metric] [-o output_mesh] [-a algorithm] [-r resoution] [-v verbosity] [-s]" << std::endl;
    std::cout << "  " << argv0 << " input_mesh [--num-samples num_samples] [--disk-samples disk_samples] [--metric metric] [--output output_mesh] [--algorithm algorithm] [--resolution resolution] [--verbosity verbosity] [--smooth]" << std::endl;
    ...
    std::cout << "    resolution is the image resolution for the output. Default is 2048." << std::endl;
